...
model = keras.models.load_model("Models/model_state.hdf5")                                              #Load a saved model.
...
while True: 												#Infinite loop.
	X = state[0] 											#Obtain input image.
	history.append(player.snake) 									#Record the curent position of the snake.
	action = model.predict_classes(X.reshape(1,1,11)) 						#Predict the action to be taken.

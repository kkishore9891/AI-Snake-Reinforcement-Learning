...
class Snake:                                                                                                        						#Creating a class for the game.
	...
	def render(self):											#Render the image of the game.
		if not self.game_over(): 									#Render only if the game is not over.
			...
			self.game[self.food[0][0],self.food[0][1]] = [255,0,100] 					#The food is yellow.
...
#If nothing is pressed, val is the same as previous value and the snake resumes going in the same direction. 			#
#If w(Top) is pressed, val = 0. If s(Bottom) is pressed, val = 1. If a(Left) is pressed, val = 2. If d(Right) is pressed, val =3. 		#
#To make the player move in the required direction, the corresponding key value is given using  player.move(x) where x=0,1,2,3. 	# 
#The move function also returns 13 values: Food distance in x and y directions and obstacle distance(wall or snake body) in all the 	#
#8 directions, the length of the snake, if the snake ate the food and if the snake hit the wall. The first 11 values are converted into an array#
#These values can also be obtained using the function player.state(). Convert the 1st 11 values to a numpy array before feeding it to the 	#
#model.                                                       	                                                                                                                                                                	#
#The function player.render() creates an open-cv window and diplays the snake, the food and the background. 			#
############################################SAMPLE######################################################
